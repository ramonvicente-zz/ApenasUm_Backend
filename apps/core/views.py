...
class SignUp(View):
    ...
    def post(self, request):
        ...
        if form.is_valid():
            ...
            if Usuario.objects.filter(Q(cpf=cpf)).exists():
                return JsonResponse({'status': 'client_error', 'message':'Usuário já registrado'}, status=400)  

            # CHECK IF USER REGISTERED
            email = request.POST.get('email')
            if User.objects.filter(username=email).exists():
                return JsonResponse({'status': 'user_error', 'message':'Email já registrado'}, status=400)  

            # CHECK PASSWORD
            password = request.POST.get('password')
            confirm_password = request.POST.get('confirm_password')
            if password != confirm_password:
                return JsonResponse({'status': 'password_error', 'message':'Senha e confirmação de senha são diferentes'}, status=400) 

            # CREATING CLIENT
            client = form.save(commit=False)
            client.user = User.objects.create_user(username=email, email=email, password=password, is_active=False)
            client.sms_code = generate_number(size=4)
